...
**Success Criteria**:
- ✅ Game runs with same functionality as before
- ✅ Code is organized into scenes
- ✅ No global variables in main.cpp
...
## 📆 **Day 1: Foundation & Code Extraction**
...
2. **Create `Lion's Den/Game/Game.cpp`**:
   - Copy `OnInit()` code from `main.cpp` → `OnGameInit()`
   - Copy `OnEvent()` code from `main.cpp` → `OnGameEvent()`
   - Copy `OnUpdate()` code from `main.cpp` → `OnGameUpdate()`
   - Copy `OnRender()` code from `main.cpp` → `OnGameRender()`
   - Copy `OnCleanUp()` code from `main.cpp` → `OnGameCleanup()`
...
**Action**: Remove duplicate code and fix issues in Engine.cpp

**Issues Found**:
- Duplicate config file reading (lines 377-408 and 410-441)
- Duplicate function calls: `OnInit()`, `OnEvent()`, `OnRenderDebugGui()`
- Duplicate variable declarations: `mouseX`, `mouseY`, `io`
- Duplicate collision processing: `Collider::ProcessAttackCollisions()`
- Duplicate layer update loop
- Unused `ExampleAppLog` struct (lines 10-111)
- `SetVideo()` function not a member function (line 129) - should be `Engine::SetVideo`
- SDL_SetRenderDrawColor has wrong parameter order (line 644): `bk.r, bk.b, bk.g` should be `bk.r, bk.g, bk.b`
- Duplicate comments throughout (many comments appear twice)
- `Window::SetMode()` called twice (lines 444-445)

**Steps**:

1. **Remove Duplicate Config Reading**:
   - Delete lines 410-441 (duplicate of 377-408)
   - Keep only one config file reading block

2. **Remove Duplicate Function Calls**:
   ```cpp
   // In Core_Init() - line 480-482:
   OnInit(); // Remove one duplicate
   
   // In Core_Event() - line 531-534:
   OnEvent(event, keyboardState); // Remove one duplicate
   
   // In Core_Render() - line 664-667:
   OnRenderDebugGui(); // Remove one duplicate
   ```

3. **Remove Duplicate Variable Declarations**:
   - In `Core_Update()`: Remove duplicate `mouseX`, `mouseY`, `io` declarations (lines 568-585)
   - Keep only the first set

4. **Remove Duplicate Collision Processing**:
   - In `Core_Update()`: Remove line 628 (duplicate of line 621)
   - Keep only one `Collider::ProcessAttackCollisions()` call

5. **Fix Duplicate Layer Update Loop**:
   ```cpp
   // BEFORE (lines 605-611):
...
7. **Fix SDL_SetRenderDrawColor Parameter Order**:
   ```cpp
   // BEFORE (line 644):
...
8. **Remove Duplicate Window::SetMode Call**:
   ```cpp
   // BEFORE (lines 444-445):
   Window::SetMode(w, h, false, "Lion's Den");
   Window::SetMode(w, h, false, "Lion's Den"); // DUPLICATE
   
   // AFTER:
   Window::SetMode(w, h, false, "Lion's Den");
   ```

9. **Remove Unused ExampleAppLog Struct**:
   - Delete lines 10-111 (entire struct definition)

10. **Remove Duplicate Comments**:
   - Remove all duplicate comment blocks throughout the file
   - Keep only one version of each comment

11. **Extract Config Reading to Helper Function** (Optional but recommended):
   ```cpp
   // Add to Engine.h private section:
   bool LoadConfig(int& width, int& height);
...
- [ ] Config file reading works
- [ ] All events process correctly
- [ ] Rendering works correctly
- [ ] No duplicate function calls

**Checkpoint**: Engine.cpp is clean, no duplicates, code compiles

---

### **Day 1 Summary**
- ✅ Code extracted from main.cpp
- ✅ Game logic in Game class
...
## 📆 **Day 2: Scene System Implementation**
...
### **Day 2 Summary**
- ✅ Scene base class implemented
- ✅ SceneManager working
- ✅ MainMenuScene created
- ✅ GameScene created
- ✅ Engine uses SceneManager
...
## 📆 **Day 3: Event System Implementation**
...
- Monster death → `MonsterKilledEvent`
- Monster spawn → `MonsterSpawnedEvent`
...
**Examples**:
- Player health changes → `HealthChangedEvent`
- Inventory changes → `InventoryChangedEvent`
- Stats update → `StatsChangedEvent`

**Testing**:
- [ ] UI updates via events
- [ ] No direct Player → UI coupling
...
## 📆 **Day 4: Component System Foundation**
...
- ✅ MovementComponent implemented
- ✅ Player refactored to use components
...
#### **Task 5.6: Optimize Engine.cpp Performance** (1-2 hours)
**Action**: Optimize frame time tracking and memory usage

**Optimizations**:

1. **Replace Vector with Deque for Frame Times**:
   ```cpp
   // Update DebugUI.h to use deque:
   ...
   if (RenderFrameTimes.size() > 100) {
       RenderFrameTimes.pop_front(); // O(1) instead of O(n)
   }
   ```

2. **Use Stack Allocation for SDL_Event**:
   ```cpp
   // BEFORE:
...
3. **Calculate DeltaTime from Actual Frame Time**:
   ```cpp
   // In Core_Update():
   static Uint32 lastFrameTime = SDL_GetTicks();
   Uint32 currentFrameTime = SDL_GetTicks();
   float deltaTime = (currentFrameTime - lastFrameTime) / 1000.0f;
   if (deltaTime <= 0.0f) deltaTime = 1.0f / 60.0f; // Fallback
...
4. **Extract Frame Time Tracking to Helper Methods**:
   ```cpp
   // Add to Engine.h:
...
5. **Remove Unused _DeleteCandidates**:
   - Remove from Engine.h line 61: `std::vector<Object*> _DeleteCandidates;`
   - It's never used (uses `Object::DeleteCandidates` instead)

**Testing**:
- [ ] Frame time tracking works correctly
- [ ] Performance improved (check frame times)
- [ ] No memory leaks
- [ ] DeltaTime calculates correctly
- [ ] Game runs smoothly

**Checkpoint**: Engine performance optimized
...
1. ✅ Code is 50%+ more maintainable (measured by complexity)
2. ✅ New features can be added 2x faster
